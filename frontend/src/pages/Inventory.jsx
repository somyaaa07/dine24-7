...
const Inventory = () => {
  ...
  useEffect(() => { fetchItems(); }, [activeTab, selectedBranchId]);
  ...
  const handleItemSubmit = async (e) => {
    ...
    try {
      if (editingItem) {
        await api.put(`/inventory/${editingItem.id}`, { ...itemForm, branch_id: selectedBranchId || undefined });
        showMsg('success', 'Item updated!');
      } else {
        await api.post('/inventory', { ...itemForm, branch_id: selectedBranchId || undefined });
        showMsg('success', 'Item added!');
      }
      setItemForm(EMPTY_ITEM);
      setEditingItem(null);
